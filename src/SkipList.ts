 ...
 */
 ...
 import {TSMT$SkipListNode } from './SkipListNodeImpl';
 import {TSMT$ISkipListData} from "./SkipListNode";
...
export class TSMT$SkipList
{
  ...
  protected _columnHead: TSMT$SkipListNode;    // reference to column root or head node
  protected _columnTail: TSMT$SkipListNode;    // reference to column tail node (this row contains the full list) - final row head node
  protected _listTail: TSMT$SkipListNode;      // tail node for the bottom row
  ...
  protected _node: TSMT$SkipListNode;
  protected _head: TSMT$SkipListNode;
  ...
  */
  ...
  public get levels(): number
  {
    ...
    let levels: number          = 1;
    let node: TSMT$SkipListNode = this._columnHead;
  ...
  /**
   * Access a row-by-row representation of the internal structure of the current list
   *
   * @returns {Array<Array<TSMT$ISkipListData>>}  Each array contains a list of TSMT$ISkipListData> that represents
   * the internal contents of each row of the skip list.  This is used primarily as debugging facility and has possible
   * use in educational applications.  This method should be used only on small-sized lists an is subject to future
   * deprecation.
   */
  public get list(): Array< Array<TSMT$ISkipListData> >
  {
    ...
    if (this._size > 0)
    {
      let head: TSMT$SkipListNode = this._columnHead;

      while (head)
      {
        let row: Array<TSMT$ISkipListData> = new Array<TSMT$ISkipListData>();
  ...
  public toArray(reverse: boolean = false): Array<TSMT$ISkipListData>
  {
    ...
    if (this._size > 0)
    {
      if (reverse)
      {
        let node: TSMT$SkipListNode = this._listTail;
      ...
      {
        let node: TSMT$SkipListNode = this._columnTail;
  ...
  /**
   * Initialize the list from an array of numerical values (in arbitrary order)
   *
   * @param {Array<number>} values
   *
   * @returns {nothing} An existing list will be overwritten by calling this method
   */
  ...
  public insert(id: string, value: number, aux?: Object): void
  {
    if (value != null && value !== undefined && id !== undefined && id != '')
    {
      ...
      let insert: TSMT$SkipListNode = this.__insertionNode(this._columnTail, value);
      ...
      else
      {
        ...
        // level up?
        let curHead: TSMT$SkipListNode = this._columnTail;
        let prev: TSMT$SkipListNode    = insert.prev;
  ...
  {
    let deletedData: TSMT$ISkipListData = null;
    ...
    if (value !== undefined && value != null)
    {
      ...
      let node: TSMT$SkipListNode = this._columnTail.next;
  ...
  /**
   * Remove the node with minimum value from the list
   *
   * @returns {TSMT$ISkipListData} Reference to data associated with the deleted node; will be null if the list is
   * empty.
   */
  public removeMin(): TSMT$ISkipListData
  {
    ...
    let toDelete: TSMT$SkipListNode = this._columnTail.next;
  ...
  /**
   * Remove the node with maximum value from the list
   *
   * @returns {TSMT$ISkipListData} Reference to data associated with the deleted node; will be null if the list is
   * empty.
   */
  public removeMax(): TSMT$ISkipListData
  {
    ...
    let toDelete: TSMT$SkipListNode = this._listTail;
  ...
  /**
   * Find a node in the list with the specified value
   *
   * @param {number} value Value to find
   *
   * @returns {TSMT$ISkipListData} Reference to node data for the found node or null if the specified value does not
   * exist in the list.
   */
  public find(value: number): TSMT$ISkipListData
  {
    // repeat cached query?
    let startNode: TSMT$SkipListNode = this._columnHead;
    if (this._node !== null)
    {
    ...
    let curHead: TSMT$SkipListNode = startNode;

    while (curHead)
    {
      let node: TSMT$SkipListNode = curHead;
  ...
  protected __newLevel(head: TSMT$SkipListNode): TSMT$SkipListNode
  {
   this._topLevel++;
    ...
    if (head == null)
    {
      ...
      let infNode: TSMT$SkipListNode = new TSMT$SkipListNode();
      infNode.key                    = 'inf';
    ...
    {
      let newHead: TSMT$SkipListNode = new TSMT$SkipListNode();
      newHead.prev  = null;
      newHead.next  = null;
      newHead.up    = null;
  ...
  protected __deleteFrom(deleteNode: TSMT$SkipListNode)
  {
    ...
    const prev: TSMT$SkipListNode = deleteNode.prev;
    const next: TSMT$SkipListNode = deleteNode.next;
  ...
  {
    let prev: TSMT$SkipListNode    = insert.prev;
    let curNode: TSMT$SkipListNode = new TSMT$SkipListNode();
    curNode.key   = id;
    curNode.value = value;
  ...
  protected __insertionNode(node: TSMT$SkipListNode, value: number): TSMT$SkipListNode
  {
