...
export interface TSMT$ISkipListNode
{
  ...
  prev: TSMT$ISkipListNode;   // previous node in list (null if head)
  next: TSMT$ISkipListNode;   // next node in list (null if tail)
  up: TSMT$ISkipListNode;     // node one level down (null if none exists)
  down: TSMT$ISkipListNode;   // node one level up in list (null if none exists)
}
